...
public final class FlowableDoFinally<T> extends AbstractFlowableWithUpstream<T, T> {
    ...
    static final class DoFinallySubscriber<T> extends BasicIntQueueSubscription<T> implements FlowableSubscriber<T> {
        ...
        public void onSubscribe(Subscription subscription) {
            if (SubscriptionHelper.validate(this.upstream, subscription)) {
                ...
                if (subscription instanceof QueueSubscription) {
                    this.f1569qs = (QueueSubscription) subscription;
                }
        ...
        public int requestFusion(int i) {
            QueueSubscription<T> queueSubscription = this.f1569qs;
            boolean z = false;
        ...
        public boolean isEmpty() {
            return this.f1569qs.isEmpty();
        }

        public T poll() throws Exception {
            T poll = this.f1569qs.poll();
    ...
    static final class DoFinallyConditionalSubscriber<T> extends BasicIntQueueSubscription<T> implements ConditionalSubscriber<T> {
        ...
        public void onSubscribe(Subscription subscription) {
            if (SubscriptionHelper.validate(this.upstream, subscription)) {
                this.upstream = subscription;
                if (subscription instanceof QueueSubscription) {
                    this.f1568qs = (QueueSubscription) subscription;
                }
                this.downstream.onSubscribe(this);
            }
        }
        ...
        public int requestFusion(int i) {
            QueueSubscription<T> queueSubscription = this.f1568qs;
            boolean z = false;
            if (queueSubscription == null || (i & 4) != 0) {
                return 0;
            }
        ...
        public boolean isEmpty() {
            return this.f1568qs.isEmpty();
        }

        public T poll() throws Exception {
            T poll = this.f1568qs.poll();
            if (poll == null && this.syncFused) {
                runFinally();
            }
            return poll;
