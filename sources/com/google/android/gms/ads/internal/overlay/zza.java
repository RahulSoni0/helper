...
public final class zza {
    ...
    public static final boolean zzb(Context context, zzc zzc, zzw zzw, zzu zzu) {
        int i = 0;
        if (zzc == null) {
            zze.zzi("No intent data for launcher overlay.");
            return false;
        }
        zzaeq.zza(context);
        ...
        if (TextUtils.isEmpty(zzc.zzb)) {
            zze.zzi("Open GMSG did not contain a URL.");
            return false;
        }
        ...
        if (!TextUtils.isEmpty(zzc.zze)) {
            ...
            if (split.length < 2) {
                ...
                zze.zzi(valueOf.length() != 0 ? "Could not parse component name from open GMSG: ".concat(valueOf) : new String("Could not parse component name from open GMSG: "));
                return false;
            }
        ...
        if (!TextUtils.isEmpty(str)) {
            try {
                ...
                zze.zzi("Could not parse intent flags.");
            }
            intent2.addFlags(i);
        }
