...
public final class zzyw {
    ...
    public final zzys zza(Context context, zzacq zzacq) {
        ...
        if (zzacq.zzt() != null) {
            zzyk = new zzyk(zzacq.zzt().getAdString(), zzzy.zzf().containsKey(zzacq.zzt().getQueryInfo()) ? zzzy.zzf().get(zzacq.zzt().getQueryInfo()) : "");
        } else {
            zzyk = null;
        }
        String zzj = zzacq.zzj();
        SearchAdRequest zzl = zzacq.zzl();
        zzadu zzadu = zzl != null ? new zzadu(zzl) : null;
        ...
        return new zzys(8, time, zzh, zzd, list, zzm, Math.max(zzacq.zzp(), zzm2.getTagForChildDirectedTreatment()), false, zzj, zzadu, zzf, zzb, zzacq.zzo(), zzacq.zzq(), Collections.unmodifiableList(new ArrayList(zzacq.zzr())), zzacq.zzk(), str, zzs, zzyk, Math.max(-1, zzm2.getTagForUnderAgeOfConsent()), (String) Collections.max(Arrays.asList(new String[]{null, zzm2.getMaxAdContentRating()}), zzyv.zza), zzacq.zzc(), zzacq.zzu());
    }
}
