...
public class CropImageActivity extends AppCompatActivity implements CropImageView.OnSetImageUriCompleteListener, CropImageView.OnCropImageCompleteListener {
    ...
    public void onCreate(Bundle bundle) {
        ...
        setContentView(C1895R.layout.crop_image_activity);
        this.mCropImageView = (CropImageView) findViewById(C1895R.C1898id.cropImageView);
        ...
        if (supportActionBar != null) {
            ...
            if (cropImageOptions == null || cropImageOptions.activityTitle == null || this.mOptions.activityTitle.length() <= 0) {
                charSequence = getResources().getString(C1895R.string.crop_image_activity_title);
            } else {
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        getMenuInflater().inflate(C1895R.C1899menu.crop_image_menu, menu);
        if (!this.mOptions.allowRotation) {
            menu.removeItem(C1895R.C1898id.crop_image_menu_rotate_left);
            menu.removeItem(C1895R.C1898id.crop_image_menu_rotate_right);
        } else if (this.mOptions.allowCounterRotation) {
            menu.findItem(C1895R.C1898id.crop_image_menu_rotate_left).setVisible(true);
        }
        if (!this.mOptions.allowFlipping) {
            menu.removeItem(C1895R.C1898id.crop_image_menu_flip);
        }
        if (this.mOptions.cropMenuCropButtonTitle != null) {
            menu.findItem(C1895R.C1898id.crop_image_menu_crop).setTitle(this.mOptions.cropMenuCropButtonTitle);
        }
        ...
        try {
            if (this.mOptions.cropMenuCropButtonIcon != 0) {
                ...
                menu.findItem(C1895R.C1898id.crop_image_menu_crop).setIcon(drawable);
            }
        } catch (Exception e) {
            Log.w("AIC", "Failed to read menu crop drawable", e);
        }
        if (this.mOptions.activityMenuIconColor != 0) {
            updateMenuItemIconColor(menu, C1895R.C1898id.crop_image_menu_rotate_left, this.mOptions.activityMenuIconColor);
            updateMenuItemIconColor(menu, C1895R.C1898id.crop_image_menu_rotate_right, this.mOptions.activityMenuIconColor);
            updateMenuItemIconColor(menu, C1895R.C1898id.crop_image_menu_flip, this.mOptions.activityMenuIconColor);
            if (drawable != null) {
                updateMenuItemIconColor(menu, C1895R.C1898id.crop_image_menu_crop, this.mOptions.activityMenuIconColor);
            }
        }
        return true;
    }
    ...
    public boolean onOptionsItemSelected(MenuItem menuItem) {
        if (menuItem.getItemId() == C1895R.C1898id.crop_image_menu_crop) {
            cropImage();
            return true;
        } else if (menuItem.getItemId() == C1895R.C1898id.crop_image_menu_rotate_left) {
            rotateImage(-this.mOptions.rotationDegrees);
            return true;
        } else if (menuItem.getItemId() == C1895R.C1898id.crop_image_menu_rotate_right) {
            rotateImage(this.mOptions.rotationDegrees);
            return true;
        } else if (menuItem.getItemId() == C1895R.C1898id.crop_image_menu_flip_horizontally) {
            this.mCropImageView.flipImageHorizontally();
            return true;
        } else if (menuItem.getItemId() == C1895R.C1898id.crop_image_menu_flip_vertically) {
    ...
    public void onRequestPermissionsResult(int i, String[] strArr, int[] iArr) {
        if (i == 201) {
            ...
            if (uri == null || iArr.length <= 0 || iArr[0] != 0) {
                Toast.makeText(this, C1895R.string.crop_image_activity_no_permissions, 1).show();
                setResultCancel();
            } else {
