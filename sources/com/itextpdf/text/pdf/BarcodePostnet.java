...
public class BarcodePostnet extends Barcode {
    ...
    public BarcodePostnet() {
        this.f1218n = 3.2727273f;
        this.f1219x = 1.4399999f;
        this.barHeight = 9.0f;
    ...
    public Rectangle getBarcodeSize() {
        return new Rectangle((((float) (((this.code.length() + 1) * 5) + 1)) * this.f1218n) + this.f1219x, this.barHeight);
    }
    ...
    public Rectangle placeBarcode(PdfContentByte pdfContentByte, BaseColor baseColor, BaseColor baseColor2) {
        ...
        for (int i = 0; i < barsPostnet.length; i++) {
            pdfContentByte.rectangle(f, 0.0f, this.f1219x - this.inkSpreading, barsPostnet[i] == b ? this.barHeight : this.size);
            f += this.f1218n;
        }
        pdfContentByte.fill();
