...
public class PdfStructureElement extends PdfDictionary implements IPdfStructureElement {
    ...
    public PdfStructureElement(PdfStructureElement pdfStructureElement, PdfName pdfName) {
        ...
        put(PdfName.f1338P, pdfStructureElement.reference);
    ...
    public PdfStructureElement(PdfStructureTreeRoot pdfStructureTreeRoot, PdfName pdfName) {
        ...
        put(PdfName.f1338P, pdfStructureTreeRoot.getReference());
        put(PdfName.TYPE, PdfName.STRUCTELEM);
    }
    ...
    private void init(PdfDictionary pdfDictionary, PdfName pdfName) {
        ...
        PdfObject pdfObject = pdfDictionary.get(PdfName.f1320K);
        if (pdfObject == null) {
            pdfArray = new PdfArray();
            pdfDictionary.put(PdfName.f1320K, pdfArray);
        } else if (pdfObject instanceof PdfArray) {
            pdfArray = (PdfArray) pdfObject;
        } else {
            PdfArray pdfArray2 = new PdfArray();
            pdfArray2.add(pdfObject);
            pdfDictionary.put(PdfName.f1320K, pdfArray2);
            pdfArray = pdfArray2;
        }
        if (pdfArray.size() > 0) {
    ...
    public PdfDictionary getParent(boolean z) {
        ...
        return (pdfStructureElement != null || !z) ? pdfStructureElement : this.top;
    }

    /* access modifiers changed from: package-private */
    public void setPageMark(int i, int i2) {
        if (i2 >= 0) {
            put(PdfName.f1320K, new PdfNumber(i2));
        }
    ...
    public void setAnnotation(PdfAnnotation pdfAnnotation, PdfIndirectReference pdfIndirectReference) {
        PdfArray asArray = getAsArray(PdfName.f1320K);
        if (asArray == null) {
            asArray = new PdfArray();
            PdfObject pdfObject = get(PdfName.f1320K);
            if (pdfObject != null) {
                asArray.add(pdfObject);
            }
            put(PdfName.f1320K, asArray);
        }
        ...
        if (pdfAnnotation.getRole() == PdfName.FORM) {
            pdfDictionary.put(PdfName.f1341PG, pdfIndirectReference);
        }
    ...
    public PdfObject getAttribute(PdfName pdfName) {
        PdfDictionary asDict = getAsDict(PdfName.f1260A);
    ...
    public void writeAttributes(IAccessibleElement iAccessibleElement) {
        ...
        if (iAccessibleElement.getAccessibleAttributes() != null) {
            for (PdfName next : iAccessibleElement.getAccessibleAttributes().keySet()) {
                if (next.equals(PdfName.f1315ID)) {
                ...
                } else if (next.equals(PdfName.LANG) || next.equals(PdfName.ALT) || next.equals(PdfName.ACTUALTEXT) || next.equals(PdfName.f1293E) || next.equals(PdfName.f1362T)) {
    ...
    private void writeAttributes(Chunk chunk) {
        ...
        if (attributes != null) {
            setAttribute(PdfName.f1332O, PdfName.LAYOUT);
    ...
    private void writeAttributes(List list) {
        if (list != null) {
            setAttribute(PdfName.f1332O, PdfName.LIST);
    ...
    private void writeAttributes(PdfPTable pdfPTable) {
        if (pdfPTable != null) {
            setAttribute(PdfName.f1332O, PdfName.TABLE);
