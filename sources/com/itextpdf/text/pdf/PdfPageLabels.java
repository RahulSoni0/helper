...
public class PdfPageLabels {
    ...
    static PdfName[] numberingStyle = {PdfName.f1282D, PdfName.f1347R, new PdfName("r"), PdfName.f1260A, new PdfName((String) HtmlTags.f1190A)};
    ...
    public void addPageLabel(int i, int i2, String str, int i3) {
        ...
        if (i2 >= 0 && i2 < numberingStyle.length) {
            pdfDictionary.put(PdfName.f1357S, numberingStyle[i2]);
        }
        if (str != null) {
            pdfDictionary.put(PdfName.f1338P, new PdfString(str, PdfObject.TEXT_UNICODE));
        }
        if (i3 != 1) {
            pdfDictionary.put(PdfName.f1359ST, new PdfNumber(i3));
        }
    ...
    public static String[] getPageLabels(PdfReader pdfReader) {
        ...
        for (int i3 = 0; i3 < numberOfPages; i3++) {
            ...
            if (readTree.containsKey(valueOf)) {
                ...
                i = pdfDictionary2.contains(PdfName.f1359ST) ? ((PdfNumber) pdfDictionary2.get(PdfName.f1359ST)).intValue() : 1;
                if (pdfDictionary2.contains(PdfName.f1338P)) {
                    str = ((PdfString) pdfDictionary2.get(PdfName.f1338P)).toUnicodeString();
                } else {
                    str = "";
                }
                c = pdfDictionary2.contains(PdfName.f1357S) ? ((PdfName) pdfDictionary2.get(PdfName.f1357S)).toString().charAt(1) : Barcode128.CODE_BC_TO_A;
            }
            if (c == 'A') {
    ...
    public static PdfPageLabelFormat[] getPageLabelFormats(PdfReader pdfReader) {
        ...
        for (int i2 = 0; i2 < numArr.length; i2++) {
            ...
            int intValue = pdfDictionary2.contains(PdfName.f1359ST) ? ((PdfNumber) pdfDictionary2.get(PdfName.f1359ST)).intValue() : 1;
            String unicodeString = pdfDictionary2.contains(PdfName.f1338P) ? ((PdfString) pdfDictionary2.get(PdfName.f1338P)).toUnicodeString() : "";
            if (pdfDictionary2.contains(PdfName.f1357S)) {
                char charAt = ((PdfName) pdfDictionary2.get(PdfName.f1357S)).toString().charAt(1);
