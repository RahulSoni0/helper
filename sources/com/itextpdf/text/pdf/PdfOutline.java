...
public class PdfOutline extends PdfDictionary {
    ...
    public void toPdf(PdfWriter pdfWriter, OutputStream outputStream) throws IOException {
        ...
        if (baseColor != null && !baseColor.equals(BaseColor.BLACK)) {
            put(PdfName.f1272C, new PdfArray(new float[]{((float) this.color.getRed()) / 255.0f, ((float) this.color.getGreen()) / 255.0f, ((float) this.color.getBlue()) / 255.0f}));
        }
        int i2 = this.style;
        if ((i2 & 1) != 0) {
            i = 2;
        ...
        if (this.action != null) {
            put(PdfName.f1260A, this.action);
        }
        if (this.count != 0) {
