...
public class PdfAction extends PdfDictionary {
    ...
    public PdfAction(String str, boolean z) {
        put(PdfName.f1357S, PdfName.URI);
    ...
    PdfAction(PdfIndirectReference pdfIndirectReference) {
        put(PdfName.f1357S, PdfName.GOTO);
        put(PdfName.f1282D, pdfIndirectReference);
    }

    public PdfAction(String str, String str2) {
        put(PdfName.f1357S, PdfName.GOTOR);
        put(PdfName.f1295F, new PdfString(str));
        put(PdfName.f1282D, new PdfString(str2));
    }

    public PdfAction(String str, int i) {
        put(PdfName.f1357S, PdfName.GOTOR);
        put(PdfName.f1295F, new PdfString(str));
        PdfName pdfName = PdfName.f1282D;
    ...
    public PdfAction(int i) {
        put(PdfName.f1357S, PdfName.NAMED);
        if (i == 1) {
            put(PdfName.f1325N, PdfName.FIRSTPAGE);
        } else if (i == 2) {
            put(PdfName.f1325N, PdfName.PREVPAGE);
        } else if (i == 3) {
            put(PdfName.f1325N, PdfName.NEXTPAGE);
        } else if (i == 4) {
            put(PdfName.f1325N, PdfName.LASTPAGE);
        } else if (i == 5) {
            put(PdfName.f1357S, PdfName.JAVASCRIPT);
            put(PdfName.f1319JS, new PdfString("this.print(true);\r"));
        } else {
    ...
    public PdfAction(String str, String str2, String str3, String str4) {
        put(PdfName.f1357S, PdfName.LAUNCH);
        if (str2 == null && str3 == null && str4 == null) {
            put(PdfName.f1295F, new PdfString(str));
            return;
        }
        PdfDictionary pdfDictionary = new PdfDictionary();
        pdfDictionary.put(PdfName.f1295F, new PdfString(str));
        if (str2 != null) {
            pdfDictionary.put(PdfName.f1338P, new PdfString(str2));
        }
        if (str3 != null) {
            pdfDictionary.put(PdfName.f1332O, new PdfString(str3));
        }
        if (str4 != null) {
            pdfDictionary.put(PdfName.f1282D, new PdfString(str4));
        }
    ...
    public static PdfAction rendition(String str, PdfFileSpecification pdfFileSpecification, String str2, PdfIndirectReference pdfIndirectReference) throws IOException {
        ...
        pdfAction.put(PdfName.f1357S, PdfName.RENDITION);
        pdfAction.put(PdfName.f1347R, new PdfRendition(str, pdfFileSpecification, str2));
    ...
    public static PdfAction javaScript(String str, PdfWriter pdfWriter, boolean z) {
        PdfAction pdfAction = new PdfAction();
        pdfAction.put(PdfName.f1357S, PdfName.JAVASCRIPT);
        String str2 = PdfObject.TEXT_UNICODE;
        if (z && str.length() < 50) {
            pdfAction.put(PdfName.f1319JS, new PdfString(str, str2));
            ...
            try {
                ...
                pdfAction.put(PdfName.f1319JS, pdfWriter.addToBody(pdfStream).getIndirectReference());
            } catch (Exception unused) {
                pdfAction.put(PdfName.f1319JS, new PdfString(str));
            }
        } else {
            pdfAction.put(PdfName.f1319JS, new PdfString(str));
        }
    ...
    static PdfAction createHide(PdfObject pdfObject, boolean z) {
        PdfAction pdfAction = new PdfAction();
        pdfAction.put(PdfName.f1357S, PdfName.HIDE);
        pdfAction.put(PdfName.f1362T, pdfObject);
        if (!z) {
            pdfAction.put(PdfName.f1304H, PdfBoolean.PDFFALSE);
        }
        return pdfAction;
    }
    ...
    public static PdfAction createSubmitForm(String str, Object[] objArr, int i) {
        PdfAction pdfAction = new PdfAction();
        pdfAction.put(PdfName.f1357S, PdfName.SUBMITFORM);
        PdfDictionary pdfDictionary = new PdfDictionary();
        pdfDictionary.put(PdfName.f1295F, new PdfString(str));
        pdfDictionary.put(PdfName.f1302FS, PdfName.URL);
        pdfAction.put(PdfName.f1295F, pdfDictionary);
        if (objArr != null) {
    ...
    public static PdfAction createResetForm(Object[] objArr, int i) {
        PdfAction pdfAction = new PdfAction();
        pdfAction.put(PdfName.f1357S, PdfName.RESETFORM);
        if (objArr != null) {
            pdfAction.put(PdfName.FIELDS, buildArray(objArr));
        }
        pdfAction.put(PdfName.FLAGS, new PdfNumber(i));
    ...
    public static PdfAction createImportData(String str) {
        PdfAction pdfAction = new PdfAction();
        pdfAction.put(PdfName.f1357S, PdfName.IMPORTDATA);
        pdfAction.put(PdfName.f1295F, new PdfString(str));
        return pdfAction;
    }
    ...
    public static PdfAction gotoLocalPage(int i, PdfDestination pdfDestination, PdfWriter pdfWriter) {
        ...
        PdfAction pdfAction = new PdfAction();
        pdfAction.put(PdfName.f1357S, PdfName.GOTO);
        pdfAction.put(PdfName.f1282D, pdfDestination2);
        return pdfAction;
    }
    ...
    public static PdfAction gotoLocalPage(String str, boolean z) {
        ...
        pdfAction.put(PdfName.f1357S, PdfName.GOTO);
        if (z) {
            pdfAction.put(PdfName.f1282D, new PdfName(str));
        } else {
            pdfAction.put(PdfName.f1282D, new PdfString(str, PdfObject.TEXT_UNICODE));
        }
        return pdfAction;
    }
    ...
    public static PdfAction gotoRemotePage(String str, String str2, boolean z, boolean z2) {
        PdfAction pdfAction = new PdfAction();
        pdfAction.put(PdfName.f1295F, new PdfString(str));
        pdfAction.put(PdfName.f1357S, PdfName.GOTOR);
        if (z) {
            pdfAction.put(PdfName.f1282D, new PdfName(str2));
        } else {
            pdfAction.put(PdfName.f1282D, new PdfString(str2, PdfObject.TEXT_UNICODE));
        }
        if (z2) {
    ...
    public static PdfAction gotoEmbedded(String str, PdfTargetDictionary pdfTargetDictionary, PdfObject pdfObject, boolean z) {
        PdfAction pdfAction = new PdfAction();
        pdfAction.put(PdfName.f1357S, PdfName.GOTOE);
        pdfAction.put(PdfName.f1362T, pdfTargetDictionary);
        pdfAction.put(PdfName.f1282D, pdfObject);
    ...
    public static PdfAction setOCGstate(ArrayList<Object> arrayList, boolean z) {
        PdfName pdfName;
        PdfAction pdfAction = new PdfAction();
        pdfAction.put(PdfName.f1357S, PdfName.SETOCGSTATE);
        PdfArray pdfArray = new PdfArray();
