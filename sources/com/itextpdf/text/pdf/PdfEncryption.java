...
public class PdfEncryption {
    ...
    private static final byte[] pad = {40, -65, 78, 94, 78, 117, -118, 65, 100, 0, 78, 86, -1, -6, 1, 8, 46, 46, 0, -74, -48, 104, DocWriter.f1152GT, ByteCompanionObject.MIN_VALUE, DocWriter.FORWARD, BidiOrder.f1235CS, -87, -2, 100, 83, 105, 122};
    ...
    public void setupAllKeys(byte[] bArr, byte[] bArr2, int i) {
        ...
        if (i2 == 5) {
            ...
            instance.update(bArr3, 0, Math.min(bArr3.length, WorkQueueKt.MASK));
            instance.update(iv);
            ...
            instance.update(bArr4, 0, Math.min(bArr4.length, WorkQueueKt.MASK));
            instance.update(iv3);
    ...
    public boolean readKey(PdfDictionary pdfDictionary, byte[] bArr) throws BadPasswordException {
        ...
        byte[] iSOBytes = DocWriter.getISOBytes(pdfDictionary.get(PdfName.f1332O).toString());
        byte[] iSOBytes2 = DocWriter.getISOBytes(pdfDictionary.get(PdfName.f1375U).toString());
        byte[] iSOBytes3 = DocWriter.getISOBytes(pdfDictionary.get(PdfName.f1334OE).toString());
        byte[] iSOBytes4 = DocWriter.getISOBytes(pdfDictionary.get(PdfName.f1376UE).toString());
        ...
        this.permissions = ((PdfNumber) pdfDictionary.get(PdfName.f1338P)).longValue();
        MessageDigest instance = MessageDigest.getInstance("SHA-256");
        instance.update(bArr, 0, Math.min(bArr.length, WorkQueueKt.MASK));
        ...
        if (processBlock[9] == 97 && processBlock[10] == 100 && processBlock[11] == 98) {
            this.permissions = (long) ((processBlock[0] & UByte.MAX_VALUE) | ((processBlock[1] & UByte.MAX_VALUE) << 8) | ((processBlock[2] & UByte.MAX_VALUE) << BidiOrder.f1242S) | ((processBlock[2] & UByte.MAX_VALUE) << 24));
    ...
    public PdfDictionary getEncryptionDictionary() {
        ...
        if (this.publicKeyHandler.getRecipientsSize() > 0) {
            ...
            pdfDictionary.put(PdfName.f1347R, new PdfNumber(this.revision));
            try {
                PdfArray encodedRecipients = this.publicKeyHandler.getEncodedRecipients();
                int i = this.revision;
                if (i == 2) {
                    pdfDictionary.put(PdfName.f1379V, new PdfNumber(1));
                    ...
                    if (i == 5) {
                        pdfDictionary.put(PdfName.f1347R, new PdfNumber(5));
                        pdfDictionary.put(PdfName.f1379V, new PdfNumber(5));
                    } else {
                        pdfDictionary.put(PdfName.f1347R, new PdfNumber(4));
                        pdfDictionary.put(PdfName.f1379V, new PdfNumber(4));
                    }
                    ...
                    if (i2 == 4) {
                        ...
                        pdfDictionary2.put(PdfName.CFM, PdfName.f1380V2);
                    }
                    PdfDictionary pdfDictionary3 = new PdfDictionary();
                    pdfDictionary3.put(PdfName.DEFAULTCRYPTFILTER, pdfDictionary2);
                    pdfDictionary.put(PdfName.f1277CF, pdfDictionary3);
                    ...
                    pdfDictionary.put(PdfName.f1379V, new PdfNumber(2));
            ...
            pdfDictionary.put(PdfName.f1332O, new PdfLiteral(StringUtils.escapeString(this.ownerKey)));
            pdfDictionary.put(PdfName.f1375U, new PdfLiteral(StringUtils.escapeString(this.userKey)));
            pdfDictionary.put(PdfName.f1338P, new PdfNumber(this.permissions));
            pdfDictionary.put(PdfName.f1347R, new PdfNumber(this.revision));
            ...
            if (i4 == 2) {
                ...
                pdfDictionary.put(PdfName.f1334OE, new PdfLiteral(StringUtils.escapeString(this.oeKey)));
                pdfDictionary.put(PdfName.f1376UE, new PdfLiteral(StringUtils.escapeString(this.ueKey)));
                pdfDictionary.put(PdfName.PERMS, new PdfLiteral(StringUtils.escapeString(this.perms)));
                pdfDictionary.put(PdfName.f1379V, new PdfNumber(this.revision));
                ...
                pdfDictionary.put(PdfName.f1277CF, pdfDictionary5);
            } else {
                if (!this.encryptMetadata) {
                    pdfDictionary.put(PdfName.ENCRYPTMETADATA, PdfBoolean.PDFFALSE);
                }
                ...
                if (this.revision == 4) {
                    ...
                    pdfDictionary6.put(PdfName.CFM, PdfName.f1380V2);
                }
                PdfDictionary pdfDictionary7 = new PdfDictionary();
                pdfDictionary7.put(PdfName.STDCF, pdfDictionary6);
                pdfDictionary.put(PdfName.f1277CF, pdfDictionary7);
            }
        }
