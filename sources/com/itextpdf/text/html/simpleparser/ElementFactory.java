...
public class ElementFactory {
    ...
    public Font getFont(ChainedProperties chainedProperties) {
        ...
        if (chainedProperties.hasProperty(HtmlTags.f1201I)) {
            i |= 2;
        }
        if (chainedProperties.hasProperty(HtmlTags.f1191B)) {
            i |= 1;
        }
        if (chainedProperties.hasProperty(HtmlTags.f1209U)) {
            i |= 4;
        }
        return this.provider.getFont(str2, str3, true, parseFloat, chainedProperties.hasProperty(HtmlTags.f1205S) ? i | 8 : i, HtmlUtilities.decodeColor(chainedProperties.getProperty(HtmlTags.COLOR)));
    }
    ...
    public List createList(String str, ChainedProperties chainedProperties) {
        List list;
        if (HtmlTags.f1210UL.equalsIgnoreCase(str)) {
            list = new List(false);
            list.setListSymbol("• ");
        } else {
            list = new List(true);
