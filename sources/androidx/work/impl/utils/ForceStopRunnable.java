...
public class ForceStopRunnable implements Runnable {
    ...
    public boolean isForceStopped() {
        if (getPendingIntent(this.mContext, PropertyOptions.DELETE_EXISTING) != null) {
            return false;
        }
        ...
        try {
            ...
            if (z) {
                for (WorkSpec next : runningWork) {
                    workSpecDao.setState(WorkInfo.State.ENQUEUED, next.f91id);
                    workSpecDao.markWorkSpecScheduled(next.f91id, -1);
                }
            }
